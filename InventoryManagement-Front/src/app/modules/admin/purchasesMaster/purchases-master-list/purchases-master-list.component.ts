...
@Component({
  selector: 'app-purchases-master-list',
  templateUrl: './purchases-master-list.component.html',
  styleUrls: ['./purchases-master-list.component.scss']
})
export class PurchasesMasterListComponent {
  frontRoute = FrontRoute;
  userId: number;

  purchasesmaster: PurchasesMaster[];
  purchasesmasters: PurchasesMaster[];

  displayedColumns: string[] = ["purchasesType","purchasesCode","warrenty","supplier","company","action"];
  dataSource: MatTableDataSource<PurchasesMaster> = new MatTableDataSource();
  ...
  constructor(
    ...
    customSubscribe = (data: any) => {
      this.purchasesmaster = data.data.data;
    ...
    load(): any {
      return this.purchasesmasterService.getSearch(
    ...
    remove(purchasesmasterId: number): void {
      if (purchasesmasterId) {
        this.messageService.confirm().then((result) => {
          if (result.value) {
            this.purchasesmasterService.delete(purchasesmasterId).subscribe((e) => {
