...
import { WareHouse } from 'src/app/features/models/wareHouse.model';
import { WareHouseService } from 'src/app/features/services/ware-house.service';
...
export class WareHouseListComponent  implements OnInit{
  ...
  displayedColumns: string[] = ["name","location","country","company","action"];
  dataSource: MatTableDataSource<WareHouse> = new MatTableDataSource();
  ...
  load(): any {
    return this.wareHouseService.getSearch(
  ...
  remove(wareHouseId: number): void {
    if (wareHouseId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.wareHouseService.delete(wareHouseId).subscribe((e) => {
