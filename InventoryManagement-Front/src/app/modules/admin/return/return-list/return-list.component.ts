...
export class ReturnListComponent implements OnInit{
  ...
  displayedColumns: string[] = ["item","quantity","reasonOfReturn","returnDate","action"];
  ...
  load(): any {
    return this.returnService.getSearch(
  ...
  remove(returnId: number): void {
    if (returnId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.returnService.delete(returnId).subscribe((e) => {
